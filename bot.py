...
class Psychonaut:
    ...
    def welcome(self):
        print(
            f"""
        {Fore.GREEN + Style.BRIGHT}Psychonaut - Daily Claim {Fore.BLUE + Style.BRIGHT}Auto BOT
            """
            f"""
        ...
        for attempt in range(retries):
            try:
                async with ClientSession(timeout=ClientTimeout(total=60)) as session:
                    ...
                    url = "http://api.capmonster.cloud/createTask"
                    data = json.dumps({
                    ...
                    async with session.post(url=url, data=data) as response:
                        ...
                        for _ in range(30):
                            res_url = "http://api.capmonster.cloud/getTaskResult"
                            res_data = json.dumps({
                                "clientKey": self.CAPTCHA_KEY,
                                "taskId": task_id
                            })
